...
Monitor API rate limits by implementing request throttling in high-volume workflows. Confluence's API has specific rate limits that vary by license tier, so implement appropriate delays between batch operations. Use the optional `cloudId` parameter when working with multiple Confluence instances to ensure proper API endpoint targeting.
...
Use the optional `cloudId` parameter when available to reduce API resolution overhead. Pre-calculate and store cloud IDs for known Confluence instances to eliminate the automatic domain-to-cloudId resolution step, improving response times for high-frequency operations.
...
### How do I retrieve content from a Confluence page in my workflow?
...
| `cloudId` | Optional Cloud ID for the instance | `<start.cloudId>` or leave empty for auto-fetch |
...
### How do I update a Confluence page with new content from my workflow?
...
### What are the correct connection tags for referencing Confluence block outputs?
...
### How do I connect a Confluence page's content to an Agent block for processing?
...
### Why is my Confluence block failing to retrieve or update pages?
...
| **Cloud ID Mismatch** | Incorrect Cloud ID specified | Leave `cloudId` empty for auto-detection |
...
### How can I build automated workflows that process Confluence documentation?
...
### How do I handle Confluence page versioning when updating content programmatically?
