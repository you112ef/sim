...
Trigger workflows from new Gmail messages. You can use the Gmail block in trigger mode to start a workflow when a new email arrives that matches your Gmail Pub/Sub notification configuration. Click the lightning bolt icon to enable trigger mode, then configure Gmail push notifications to your webhook URL via Google Cloud Pub/Sub.

### Configuration Inputs

| Input | Type | Required | Description |
|------|------|----------|-------------|
| `subscription` | string | Yes | Pub/Sub subscription that delivers Gmail notifications to your webhook. |
| `validateToken` | boolean | No | Validate Pub/Sub verification tokens and message signatures. Defaults to `true`. |
| `includeMessagePayload` | boolean | No | Fetch and include full message data after notification. Defaults to `false`. |

### Outputs

| Output | Type | Description |
|--------|------|-------------|
| `payloads` | array | The raw Pub/Sub notifications received. |
| `latestPayload` | object | Convenience object of the latest Pub/Sub notification. |
| `gmailEvent` | object | Parsed Gmail notification fields (email id, thread id, label ids). |

#### `latestPayload` shape

| Field | Type | Description |
|-------|------|-------------|
| `message.messageId` | string | Unique Pub/Sub message ID. |
| `message.data` | string | Base64-encoded Gmail notification data. |
| `message.publishTime` | string | RFC3339 timestamp when Pub/Sub published the message. |
| `subscription` | string | Pub/Sub subscription that received the message. |

### Example webhook event body (raw Gmail Pub/Sub payload)
...
```json
{
  "message": {
    ...
    "data": "eyJlbWFpbEFkZHJlc3MiOiAieW91QGV4YW1wbGUuY29tIiwgImhpc3RvcnlJZCI6ICIxNzM1MDEyMzQ1Njc4OSIsICJtZXNzYWdlSWQiOiAiMTIzNDU2Nzg5MDEyMyIsICJsYWJlbElkcyI6IFsiSU5CT1giLCAiVU5SRUFEIl19"
  },
  "subscription": "projects/my-project/subscriptions/gmail-webhook"
}
```

Decoded `message.data` example:

```json
{
  "emailAddress": "you@example.com",
  "historyId": "17350123456789",
  "messages": [
    { "id": "1234567890123", "threadId": "222333444555" }
  ],
  "labelIds": ["INBOX", "UNREAD"]
}
```

Method: `POST`  •  Headers: `Content-Type: application/json`, `ce-type: google.cloud.pubsub.topic.v1.messagePublished`
...
<Accordions type="single">
  <Accordion title="How do I send an email from my workflow using the Gmail block?">
  ...
  <Accordion title="How do I trigger a workflow when I receive a new Gmail email?">
  ...
  <Accordion title="What are the correct connection tags for referencing Gmail block outputs?">
  ...
  <Accordion title="How do I read specific emails using the Gmail block?">
  ...
  <Accordion title="How do I create draft emails without sending them immediately?">
  ...
  <Accordion title="Why is my Gmail block failing to authenticate or send emails?">
  ...
  <Accordion title="Can I combine Gmail with other Sim.ai blocks for email automation?">
