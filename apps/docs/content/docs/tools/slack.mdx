...
Trigger workflows from new Slack channel messages. You can use the Slack block in trigger mode to start a workflow when your Slack app receives an event (e.g., `message.channels`) via Slack Event Subscriptions. Click the lightning bolt icon to enable trigger mode, then configure your app’s Events to send to your workflow webhook URL.

### Configuration Inputs

| Input | Type | Required | Description |
|------|------|----------|-------------|
| `eventType` | string | Yes | Slack event type to handle (e.g., `message.channels`). |
| `signingSecret` | string | No | Slack app signing secret for signature validation. |
| `validateSignature` | boolean | No | Validate `X-Slack-Signature` using `X-Slack-Request-Timestamp`. Defaults to `true`. |

### Outputs

| Output | Type | Description |
|--------|------|-------------|
| `payloads` | array | The raw Slack event payloads received. |
| `latestPayload` | object | Convenience object of the latest event payload. |
| `slackEvent` | object | Parsed event fields (type, channel, user, text, ts). |

#### `latestPayload` shape

| Field | Type | Description |
|-------|------|-------------|
| `type` | string | Envelope type (e.g., `event_callback`). |
| `team_id` | string | Slack team ID. |
| `api_app_id` | string | App ID. |
| `event` | object | The inner event object (e.g., `message`). |
| `event_id` | string | Unique event ID. |
| `event_time` | number | Unix timestamp for event. |

### Example webhook event body (raw Slack event payload)
...
Method: `POST`  •  Headers: `Content-Type: application/json`, `X-Slack-Signature`, `X-Slack-Request-Timestamp`
...
<Accordions type="single">
  <Accordion title="How do I send a message from my workflow to a Slack channel?">
  ...
  <Accordion title="How do I trigger a workflow when someone mentions my bot in Slack?">
  ...
  <Accordion title="What is the correct way to reference Slack block outputs in my workflow?">
  ...
  <Accordion title="How do I connect an Agent block's output to a Slack message with formatting?">
  ...
  <Accordion title="How do I read messages from a Slack channel in my workflow?">
  ...
  <Accordion title="Why is my Slack message not sending or canvas not creating?">
  ...
  <Accordion title="Can I create complex Slack automations with multiple operations?">
