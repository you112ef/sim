...
Trigger workflows from new Outlook messages. You can use the Outlook block in trigger mode to start a workflow when new email arrives in a mailbox or folder using Microsoft Graph change notifications (webhooks). Click the lightning bolt icon to enable trigger mode, then configure a Graph subscription targeting `messages` to deliver notifications to your workflow webhook URL.

### Configuration Inputs

| Input | Type | Required | Description |
|------|------|----------|-------------|
| `resource` | string | Yes | Microsoft Graph resource to monitor (e.g., `/me/messages` or `/users/{id}/mailFolders('Inbox')/messages`). |
| `clientState` | string | No | Optional client state to validate notifications. Returned as-is in each notification. |
| `includeResourceData` | boolean | No | Include encrypted resource data in notifications (requires additional setup). Defaults to `false`. |

### Outputs

| Output | Type | Description |
|--------|------|-------------|
| `payloads` | array | The raw Microsoft Graph notification payloads received. |
| `latestPayload` | object | Convenience object of the latest notification. |
| `outlookEvent` | object | Parsed notification fields (subscription id, change type, resource, message id). |

#### `latestPayload` shape

| Field | Type | Description |
|-------|------|-------------|
| `value[].subscriptionId` | string | ID of the Graph subscription. |
| `value[].changeType` | string | Type of change (e.g., `created`, `updated`, `deleted`). |
| `value[].resource` | string | Resource path that changed (e.g., `Users/{id}/Messages/{messageId}`). |
| `value[].resourceData.id` | string | ID of the changed message (when resource data is included). |
| `value[].clientState` | string | Client-provided state used for validation. |
| `value[].subscriptionExpirationDateTime` | string | When the subscription expires. |

### Example webhook event body (raw Outlook/Microsoft Graph payload)

```json
{
  "value": [
    {
      "subscriptionId": "f3f9b1be-1a2b-4e62-8f4c-bc0b1b2a7b9a",
      "tenantId": "00000000-0000-0000-0000-000000000000",
      "changeType": "created",
      "resource": "Users/12345678-90ab-cdef-1234-567890abcdef/Messages/AAMkAGI2...AAA=",
      "subscriptionExpirationDateTime": "2023-01-01T01:00:00.000Z",
      "clientState": "secretClientState",
      "resourceData": {
        "@odata.type": "#Microsoft.Graph.Message",
        "@odata.id": "Users/12345678-90ab-cdef-1234-567890abcdef/Messages/AAMkAGI2...AAA=",
        "id": "AAMkAGI2...AAA=",
        "webUrl": "https://outlook.office365.com/mail/deeplink/compose/AAMkAGI2...AAA="
      }
    }
  ]
}
...
Method: `POST`  •  Headers: `Content-Type: application/json`, `ValidationToken` (for initial validation callbacks)
...
<Accordions type="single">
  <Accordion title="How do I send an email from my workflow using the Outlook block?">
  ...
  <Accordion title="How do I create email drafts instead of sending them immediately?">
  ...
  <Accordion title="What is the correct way to reference Outlook block outputs in my workflow?">
  ...
  <Accordion title="How do I read emails from Outlook and process them in my workflow?">
  ...
  <Accordion title="How do I connect an Agent block's output to compose Outlook emails?">
  ...
  <Accordion title="Why is my Outlook block failing to authenticate or send emails?">
  ...
  <Accordion title="Can I use Outlook with other Sim.ai blocks for email automation?">
