...
条件に基づいてデータを異なるエージェントに送信するルーティングロジックを使用するワークフロー：
...
ツールを使用してウェブを検索し、構造化データを返すワークフロー：
...
ループを使用してコレクション内の各アイテムを処理するワークフロー：
