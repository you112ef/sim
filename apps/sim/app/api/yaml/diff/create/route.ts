...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (result.success && result.diff?.proposedState) {
      ...
      // Preserve positions from baseline for unchanged/edited blocks
      try {
        preserveBlockPositions(result.diff.proposedState, currentWorkflowState, result.diff.diffAnalysis)
      } catch (e) {
        logger.warn(`[${requestId}] Position preservation failed`, e as any)
      }
    }
    ...
    if (result.success && result.blocks && !result.diff) {
      ...
      // Preserve positions from baseline for unchanged/edited blocks
      try {
        preserveBlockPositions(transformedResult.diff.proposedState, currentWorkflowState, diffAnalysis)
      } catch (e) {
        logger.warn(`[${requestId}] Position preservation failed (transformed)`, e as any)
      }
